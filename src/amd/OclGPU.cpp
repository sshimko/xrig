...
static inline void port_sleep(uint32_t sec)
{
    Sleep(sec * 1000);
}
#else
...
#   endif
    ...
    cl_context opencl_ctx = clCreateContext(nullptr, (cl_uint)num_gpus, TempDeviceList, nullptr, nullptr, &ret);
    if(ret != CL_SUCCESS) {
    ...
    for (int i = 0; i < num_gpus; ++i) {
        if ((ret = (cl_int)InitOpenCLGpu(i, opencl_ctx, &ctx[i], source_code.c_str())) != OCL_ERR_SUCCESS) {
            return ret;
        }
    }
...
size_t XMRRunJob(GpuContext* ctx, cl_uint* HashOutput)
{
    ...
    const size_t g_intensity = ctx->rawIntensity;
    const size_t w_size = ctx->workSize;
    ...
    if ((ret = clEnqueueWriteBuffer(ctx->CommandQueues, ctx->ExtraBuffers[2], CL_FALSE, sizeof(cl_uint) * g_intensity, sizeof(cl_uint), &zero, 0, NULL, NULL)) != CL_SUCCESS)
    {
        LOG_ERR("Error %s when calling clEnqueueWriteBuffer to zero branch buffer counter %d.", err_to_str(ret), 0);
        return OCL_ERR_API;
    }

    if ((ret = clEnqueueWriteBuffer(ctx->CommandQueues, ctx->ExtraBuffers[3], CL_FALSE, sizeof(cl_uint) * g_intensity, sizeof(cl_uint), &zero, 0, NULL, NULL)) != CL_SUCCESS)
    {
        LOG_ERR("Error %s when calling clEnqueueWriteBuffer to zero branch buffer counter %d.", err_to_str(ret), 1);
        return OCL_ERR_API;
    }

    if ((ret = clEnqueueWriteBuffer(ctx->CommandQueues, ctx->ExtraBuffers[4], CL_FALSE, sizeof(cl_uint) * g_intensity, sizeof(cl_uint), &zero, 0, NULL, NULL)) != CL_SUCCESS)
    {
        LOG_ERR("Error %s when calling clEnqueueWriteBuffer to zero branch buffer counter %d.", err_to_str(ret), 2);
        return OCL_ERR_API;
    }

    if ((ret = clEnqueueWriteBuffer(ctx->CommandQueues, ctx->ExtraBuffers[5], CL_FALSE, sizeof(cl_uint) * g_intensity, sizeof(cl_uint), &zero, 0, NULL, NULL)) != CL_SUCCESS)
    {
        LOG_ERR("Error %s when calling clEnqueueWriteBuffer to zero branch buffer counter %d.", err_to_str(ret), 3);
        return OCL_ERR_API;
    }
